...
from src.modules.core.domain.interfaces.icompanies_repository import ICompaniesRepository
...
class CompaniesRepository(ICompaniesRepository):
    async def list(self) -> Optional[List[Company]]:
        async with DbConnectionHandler() as database:
            try:
                if database.session:
                    result = await database.session.execute(select(Company))
        ...
        async with DbConnectionHandler() as database:
            try:
                ...
                if database.session:
                    ...
                    company_role_user = UserCompanyRole(user_id=UUID(owner_id), company_id=new_register.id, is_owner=True, role_id=None)
    ...
    async def delete(self, id: str) -> None:
        async with DbConnectionHandler() as database:
            try:
                if database.session:
                    await database.session.execute(delete(Company).where(CompanyModel.id == UUID(id)))
                    await database.session.commit()
            except Exception as exception:
                if database.session:
                    await database.session.rollback()
